...
#number of disease regarding to genes
...
for i in range(0, len(case_gene_filter_labeled)):
    ...
    for j in range(0, len(gene_coexpression_data)):
        if str(case_gene_filter_labeled['\\11_Candidate genes\\Gene Name\\'].iloc[i]).upper() == str(gene_coexpression_data["Gene Symbol"].iloc[j]).upper():
            flag = 1
...
#         if str(case_gene_filter_labeled['\\11_Candidate genes\\Gene Name\\'].iloc[i]).upper() == str(gene_coexpression_id['GeneA'].iloc[j]).upper():
#             count = count+ 1
